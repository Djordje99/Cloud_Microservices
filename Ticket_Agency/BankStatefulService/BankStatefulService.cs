using BankStatefulService.Services;
...
namespace BankStatefulService
{
    ...
    /// </summary>
    internal sealed class BankStatefulService : StatefulService
    {
        ...
        private ICommunicationListener CreateWcfCommunicationListener(StatefulServiceContext context)
        {
            ...
            var endpointConfig = context.CodePackageActivationContext.GetEndpoint("BankService");
            int port = endpointConfig.Port;
            var scheme = endpointConfig.Protocol.ToString();
            string url = string.Format(CultureInfo.InvariantCulture, "net.{0}://{1}:{2}/BankService", scheme, host, port);

            WcfCommunicationListener<IBankService> listenet = null;

            listenet = new WcfCommunicationListener<IBankService>(
            serviceContext: context,
            wcfServiceObject: new BankService(this.StateManager),
        ...
        protected override async Task RunAsync(CancellationToken cancellationToken)
        {
            ...
            {
                IBankService departureService = null;
                try
                {
